    ...
    for (auto & [ctx_key, ctx_ptr] : ml.ctx_map) {
        ...
        if (ml.borrowed_buffer_data != nullptr && ggml_backend_dev_type(dev) == GGML_BACKEND_DEVICE_TYPE_CPU) {
            ...
            for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
                const int64_t tid = gguf_find_tensor(ml.metadata, ggml_get_name(t));
                if (tid < 0) {
                    throw std::runtime_error(format("tensor '%s' not found in the model", ggml_get_name(t)));
                }
                const size_t offset = gguf_get_data_offset(ml.metadata) + gguf_get_tensor_offset(ml.metadata, tid);
                const size_t nbytes = ggml_nbytes(t);
                if (offset > ml.borrowed_buffer_size || nbytes > ml.borrowed_buffer_size - offset) {
                    throw std::runtime_error(format("tensor '%s' is outside the borrowed buffer", ggml_get_name(t)));
                }
                const auto external = ml.borrowed_tensor_views.find(ggml_get_name(t));
                const void * source = external == ml.borrowed_tensor_views.end()
                    ? ml.borrowed_buffer_data + offset : external->second.data;
                if (external != ml.borrowed_tensor_views.end() && external->second.size != nbytes) {
                    throw std::runtime_error(format("tensor '%s' borrowed view size mismatch", ggml_get_name(t)));
                }
